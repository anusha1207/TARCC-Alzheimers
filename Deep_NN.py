...
def DNN_One(input, output, epochs=200, batch_size=5, n_split=5):
    ...
    cat_output = np.delete(cat_output, 0, axis=1)
...
def DNN_Four(input, output, epochs=200, batch_size=5, n_split=5):
  ...
  def DNN_model():
    ...
    # First layer of nodes of number of columns/features in dataset and then hidden layer of 128 nodes
    model.add(Dense(128, input_shape=(len(input.columns), ), activation='relu'))
    # Another hidden layer of 128 nodes
...
def DNN_Five(input, output, epochs=200, batch_size=5, n_split=5):
  # Data Processing on Input so that we can fit the input into our neural network training
  input_values = input.values
  input_float = input_values.astype(float)
  ...
  def DNN_model():
    model = Sequential()
    # First layer of nodes of number of columns/features in dataset and then hidden layer of 512 nodes
    model.add(Dense(512, input_shape=(len(input.columns), ), activation='relu'))
    # Another hidden layer of 1024 nodes
    model.add(Dense(1024, activation='relu'))
    # Another hidden layer of 1024 nodes
    model.add(Dense(1024, activation='relu'))
    # Another hidden layer of 1024 nodes
    model.add(Dense(1024, activation='relu'))
    # Another hidden layer of 512 nodes
    model.add(Dense(512, activation='relu'))
    # Output layer of number of output classes (number of unique values of 'P1_PT_TYPE' column/feature)
    model.add(Dense(len(output.unique()), activation='softmax'))
    # Use categorical_crossentropy for loss since this is a classification problem
    # Use ADAM optimizer since we are dealing with a large dataset (in terms of number of rows and features)
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    return model
...
def DNN_Six(input, output, epochs=200, batch_size=5, n_split=5):
  # Data Processing on Input so that we can fit the input into our neural network training
  input_values = input.values
  input_float = input_values.astype(float)
  ...
  def DNN_model():
    model = Sequential()
    # First layer of nodes of number of columns/features in dataset and then hidden layer of 1024 nodes
    model.add(Dense(1024, input_shape=(len(input.columns), ), activation='relu'))
    # Output layer of number of output classes (number of unique values of 'P1_PT_TYPE' column/feature)
    model.add(Dense(len(output.unique()), activation='softmax'))
    # Use categorical_crossentropy for loss since this is a classification problem
    # Use ADAM optimizer since we are dealing with a large dataset (in terms of number of rows and features)
